...
.PARAMETER ZwaveJsCredential
    ...
    fresh. Prefer this over -ZwaveJsToken.

    Use (Get-Credential) to be prompted for both, or Import-CliXml for an
    unattended run. From a POSIX shell, prefer -ZwaveJsUser: the shell parses
    the command line before PowerShell does and chokes on the parentheses.

.PARAMETER ZwaveJsUser
    zwave-js-ui username to log in as, for example "admin". The password is
    prompted for, so it never reaches your shell history or the process list.
    This is the form to use when launching the script from bash or zsh as
    `pwsh ./Rename-Domoticz-From-ZwaveJSON.ps1 ...`, where a (Get-Credential)
    sub-expression would be parsed by the shell rather than by PowerShell.
    Needs a console to prompt at; for an unattended run use -ZwaveJsCredential
    with Import-CliXml instead. Cannot be combined with -ZwaveJsCredential.
...
.EXAMPLE
    .\Rename-Domoticz-From-ZwaveJSON.ps1 -ZwaveJsUrl "http://zwave-host:8091" -DbPath "domoticz.db" -ZwaveJsUser admin -DryRun

    Same preview against a zwave-js-ui that has authentication enabled. Only the
    password is prompted for. This form also works when the script is launched
    from a POSIX shell as `pwsh ./Rename-Domoticz-From-ZwaveJSON.ps1 ...`, where
    (Get-Credential) would be eaten by the shell instead of PowerShell.
...
param (
    ...
    [Parameter(ParameterSetName = 'FromZwaveJs',
        HelpMessage = "zwave-js-ui username to log in as; the password is prompted for")]
    [string]$ZwaveJsUser,
...
#region Version
# The startup banner and the HTML report footer both read this. They each
...
function New-HtmlReport {
    ...
    if ($RenameList -and $RenameList.Count -gt 0) {
        # A single RenameList entry always has exactly one Unit's worth of old
        # names, whether the device behind it has one Domoticz row or several,
        # so an entry's own shape cannot tell those cases apart. What can: a
        # multi-unit device that was split per unit contributes MORE THAN ONE
        # entry sharing the same DeviceID, while a genuine single-row device
        # never repeats its DeviceID here, even when its own Unit number
        # happens to be nonzero. Count occurrences up front.
        $deviceIdCounts = @{}
        foreach ($it in $RenameList) {
            if ($deviceIdCounts.ContainsKey($it.DeviceID)) { $deviceIdCounts[$it.DeviceID]++ } else { $deviceIdCounts[$it.DeviceID] = 1 }
        }

        foreach ($item in $RenameList) {
            $deviceId = [System.Web.HttpUtility]::HtmlEncode($item.DeviceID)
            # PowerShell unwraps a single-element array to its element in a boolean
            # test, so "$item.Units -and ..." would be $false for unit 0 (0 itself
            # is falsy) and silently hide the annotation for the most common unit
            # number. Test for $null explicitly instead of relying on truthiness.
            $unitSuffix = if ($deviceIdCounts[$item.DeviceID] -gt 1 -and $null -ne $item.Units -and @($item.Units).Count -eq 1) { " (unit $(@($item.Units)[0]))" } else { "" }
            ...
            <div class="device-item" data-changes="$dataChanges">
                <div class="device-header" onclick="toggleDevice(this)">
                    ...
                    <div class="device-name">
                        ...
                        <div class="device-id">$deviceId$unitSuffix</div>
                    </div>
                    <div class="changes">
                        $badges
                    </div>
...
$deviceItemsHtml
        ...
        <footer>
            Generated by Rename-Domoticz-From-ZwaveJSON.ps1 v$($Script:Version)
        </footer>
    </div>
...
$bannerText = "Domoticz Device Renamer from Z-Wave JSON Export v$($Script:Version)"
$bannerPad = 62 - $bannerText.Length
$bannerLeft = [math]::Ceiling($bannerPad / 2)
Write-Host ("║" + (' ' * $bannerLeft) + $bannerText + (' ' * ($bannerPad - $bannerLeft)) + "║") -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
if ($PSCmdlet.ParameterSetName -eq 'FromZwaveJs') {
    # -ZwaveJsUser is the shell-safe way in: (Get-Credential) is a
    # sub-expression, and a POSIX shell parses the command line before
    # PowerShell does when the script is launched as `pwsh script.ps1 ...`.
    # Prompting here rather than through a [Credential()] transformation
    # attribute on -ZwaveJsCredential is deliberate: a transformation that
    # cannot prompt aborts during parameter binding, before this script runs at
    # all, and PowerShell exits 0. A silent success that renamed nothing is the
    # one outcome this tool's exit codes must never produce.
    if ($ZwaveJsUser) {
        if ($ZwaveJsCredential) {
            Write-Host "  ❌ ERROR: Supply either -ZwaveJsUser or -ZwaveJsCredential, not both." -ForegroundColor Red
            exit $Script:ExitCodes.Error
        }
        if ([Console]::IsInputRedirected) {
            Write-Host "  ❌ ERROR: -ZwaveJsUser has to prompt for a password, and this run has no console to prompt at." -ForegroundColor Red
            Write-Host "     For an unattended run, save the credential once:" -ForegroundColor Gray
            Write-Host "       Get-Credential | Export-CliXml ./zwave.cred" -ForegroundColor Yellow
            Write-Host "     then pass -ZwaveJsCredential (Import-CliXml ./zwave.cred)." -ForegroundColor Gray
            exit $Script:ExitCodes.Error
        }
        $ZwaveJsCredential = Get-Credential -UserName $ZwaveJsUser -Message "Password for zwave-js-ui at $ZwaveJsUrl"
        if (-not $ZwaveJsCredential) {
            Write-Host "  ❌ ERROR: No password entered for '$ZwaveJsUser'." -ForegroundColor Red
