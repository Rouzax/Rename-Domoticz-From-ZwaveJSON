...
| **"Name collision detected"** | Collisions across different endpoints are auto-resolved with an endpoint suffix; unresolvable collisions (same endpoint, or against a device keeping its current name) are reported and skipped rather than written. See [Name collisions](using/collisions.md). |
